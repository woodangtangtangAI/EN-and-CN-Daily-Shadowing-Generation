...
# Title
title = doc.add_heading('YouTube Shadowing Practice', 0)
title.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER

# Original Link
p_link = doc.add_paragraph()
p_link.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
p_link.paragraph_format.space_after = Pt(12)
run_lbl = p_link.add_run("🔗 원본 유튜브 링크 (Original Link): ")
run_lbl.bold = True
run_url = p_link.add_run("https://www.youtube.com/watch?v=oJhU1IGTdoo&list=PL1OnCndYUckbQjCBlqFLPxn4mM7QgTNaG&index=18")
run_url.font.color.rgb = RGBColor(0, 102, 204)
run_url.underline = True

p_intro = doc.add_paragraph("화자별로 영어 스크립트 원문이 먼저 나오고, 바로 아래에 한글 해석이 이어지도록 구성하여 가독성을 높였습니다.")
p_intro.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT

doc.add_heading('1. Dialog Script', level=1)

for idx, (speaker, eng, kor) in enumerate(dialogue, 1):
    # Speaker heading
    p_speaker = doc.add_paragraph()
    p_speaker.paragraph_format.space_before = Pt(12)
    p_speaker.paragraph_format.space_after = Pt(2)
    run_spk = p_speaker.add_run(f"[{idx}] {speaker}")
    run_spk.bold = True
    run_spk.font.size = Pt(11)
    
    if speaker == "Lucas":
        run_spk.font.color.rgb = RGBColor(0, 51, 153) # Dark Blue
    elif speaker == "Sophia":
        run_spk.font.color.rgb = RGBColor(153, 0, 76) # Dark Pink/Purple
    else:
        run_spk.font.color.rgb = RGBColor(51, 51, 51) # Dark Gray
        
    # English Text
    p_eng = doc.add_paragraph()
    p_eng.paragraph_format.left_indent = Inches(0.2)
    p_eng.paragraph_format.space_after = Pt(2)
    run_en_lbl = p_eng.add_run("EN: ")
    run_en_lbl.bold = True
    p_eng.add_run(eng)
    
    # Korean Text
    p_kor = doc.add_paragraph()
    p_kor.paragraph_format.left_indent = Inches(0.2)
    p_kor.paragraph_format.space_after = Pt(8)
    run_ko_lbl = p_kor.add_run("KO: ")
    run_ko_lbl.bold = True
    run_ko_lbl.font.color.rgb = RGBColor(100, 100, 100)
    run_kor_text = p_kor.add_run(kor)
    run_kor_text.font.color.rgb = RGBColor(80, 80, 80) # Charcoal gray for Korean translation

# Spacing before next section
doc.add_paragraph()

doc.add_heading('2. Key Vocabulary & Expressions', level=1)
...
for word, meaning in vocab_list:
    ...
    run = p.add_run(word + " : ")
    run.bold = True
    run.font.color.rgb = RGBColor(0, 102, 204) # Highlighted vocabulary
    p.add_run(meaning)
...
# Save document
output_path = os.path.join(target_dir, "Shadowing_Practice.docx")
